...
function getFileExtension(filename) {
  ...
  return ( filename.lastIndexOf('.') > 0 ) ? filename.substring(filename.lastIndexOf('.')).toLowerCase() : '';
}

// getFileExtension('cat.jpg');
// ========== 任務三：解析檔案 metadata ==========
/**
...
function handleUpload(req, res, config) {
  ...
  // 1. 監聽解析錯誤
  form.on('error', (err) => {
    console.error('解析過程發生錯誤：', err.message);
  });
  ...
  form.parse( req, ( err, fields, files ) => {
    ...
    if( !file || !file.originalFilename ) {
      handleErrorResponse(res, 400, 'No file uploaded');
      return;
    }

    const metaData = parseFileMetadata(file);
    if( metaData.ext === '' ) {
      handleErrorResponse(res, 400, '不支援的檔案格式');
      return
    }
...
function handleErrorResponse(res, status, errorMessage) {
 ...
 */
// createUploadServer({ uploadDir: '/tmp', maxFileSize: 5 * 1024 * 1024 });
